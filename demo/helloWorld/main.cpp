...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	cbox.AddOption("0", L"-请选择-");
	cbox.AddOption("1", L"青菜");
	cbox.AddOption("2", L"牛肉");
	cbox.AddOption("3", L"猪肉");
	cbox.SetValue("2");
	...
	btn.EventHandler = [&text, &btn](Control* sd, const EventArgs& arg)->void {
		if (arg.EventType == Event::OnMouseDown) {
			text.SetMultiLine(!text.IsMultiLine());
			btn.SetText(btn.GetCheck() ? L"已选中" : L"未选中");
			text.Invalidate();
			//btn.SetEnabled(btn.GetCheck());
		}
		};

	btn.Style.ForeColor = Color::White;

	btn.DisabledStyle.BackColor = Color::Blue;//禁用状态下背景色
	btn.CheckedStyle.BackColor = Color::Purple;//如果是选择框 被选中的背景色

	btn.Style.BackColor = Color::Black;//默认背景色
	btn.HoverStyle.BackColor = Color::Red;//悬浮背景色
	btn.ActiveStyle.BackColor = Color::Green;//按下背景色
	btn.FocusStyle.BackColor = Color::Gray;//具有焦点时的背景色
