...
namespace EzUI {
	...
	void Label::OnForePaint(PaintEventArgs& args)
	{
		...
		if (!_wstr.empty()) {
			...
			args.Painter.DrawString(_wstr, GetFontFamily(this->State).utf16(), GetFontSize(this->State), GetForeColor(this->State), Rect(0, 0, (float)Width(), (float)Height()), TextAlign, _underline);
		}
	}
