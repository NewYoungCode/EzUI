...
namespace EzUI {
	...
	protected:
		D2D_COLOR_F ToColorF(const Color& color) {
			...
			byte g = color.GetG();
			byte b = color.GetB();
			float aF = (a == 255 ? (float)1.0 : a * (float)0.003922);
			float rF = (r == 255 ? (float)1.0 : r * (float)0.003922);
			float gF = (g == 255 ? (float)1.0 : g * (float)0.003922);
			float bF = (b == 255 ? (float)1.0 : b * (float)0.003922);
