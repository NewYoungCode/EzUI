...
namespace ezui {
	...
	public:
		...
		//修改已有的菜单项文字
		void SetText(UINT_PTR id, const UIString& text);
		...
		//显示菜单
		void Show(WindowHandle ownerHWnd, Point position);
	};
	...
	public:
		...
		//获取到上级菜单(如果没有则返回NULL)
		PopupMenu* GetOwnerMenu();

		//获取到根菜单(关闭根菜单则整个弹出菜单都会关闭)
		PopupMenu* GetRootMenu();

		/// 添加一个子菜单项
		Item* Append(const UIString& text, Image* icon = NULL);

		/// 添加一个子菜单
		Item* Append(PopupMenu* subMenu, const UIString& subMenuText, Image* icon = NULL);
