...
namespace ezui {
	//控件集合
	typedef Collection<Control*> ControlCollection;

	//描述控件行为
	EZUI_SCOPED_ENUM_BEGIN(ControlAction, int16_t) {
		None,
			...
			Max,//最大化|恢复
			Close//关闭
	} EZUI_SCOPED_ENUM_END(ControlAction)

			// size策略 优先级(Fixed > Auto > Percent > Stretch)
			EZUI_SCOPED_ENUM_BEGIN(SizePolicy, int16_t)
		{
			// 固定尺寸：不随布局变化
			Fixed,
				// 内容自适应：由内容决定尺寸
				Auto,
				// 百分比：按父容器总尺寸比例计算（parent * ratio）
				Percent,
				// 拉伸：占用父容器剩余空间（flex）(在布局容器中 此为默认)
				Stretch
		} EZUI_SCOPED_ENUM_END(SizePolicy)
				 ...
				 */
				EZUI_SCOPED_ENUM_BEGIN(Flags, int16_t) {
					Visible = 1 << 0,				///< 控件是否可见（父控件隐藏时子控件也不可见）
						...
						DropEnabled = 1 << 9			///< 是否接收文件拖拽事件
				} EZUI_SCOPED_ENUM_END(Flags)
			...
			public:
				CtrlWeakPtr(T* obj = (T*)(NULL)) : WeakPtr<T>(obj, obj ? obj->GetAlive() : NULL) {
				}
			};

			/// <summary>
			/// 所有控件的基类。
			/// 控件树关系与 Object 所有权关系相互独立：控件树负责布局、绘制和事件路由，
			/// Object 所有权只负责对象生命周期。普通控件加入控件树时默认不转移所有权，
			/// 仅在 takeOwnership 为 true 或显式调用 Attach() 时由拥有者负责销毁；
			/// 弹簧控件由所在父控件自动管理生命周期。
			...
			private:
				struct ControlContext {
					...
					/// 内部样式表集合（用于选择器匹配）。
					std::list<StyleRule>* m_styles;
					//绑定的事件集合
					std::list<detail::EventHandlerEntry>* m_eventHandlers;
					/// 控件名称（可用于查找控件）。
					UIString* m_name;
					/// 元素类名集合（可用于样式选择器匹配）。
					std::vector<UIString>* m_classNames;
					ControlContext();
				};
				...
				public:
					...
					ControlCollection* m_drawnCtrls;

					RenderCache()EZUI_NOEXCEPT;

					// 将一个已绘制控件加入内部缓存
					void RecordDrawn(Control* ctrl)EZUI_NOEXCEPT;

					// 从内部缓存移除一个已绘制控件
					void UnrecordDrawn(Control* ctrl)EZUI_NOEXCEPT;

					// 清空所有已绘制控件缓存
					void ClearDrawnCache()EZUI_NOEXCEPT;
				};
				...
				public:
					...
					FloatCache()EZUI_NOEXCEPT;

					// 将浮动控件加入内部缓存
					void RecordFloat(Control* floatCtrl)EZUI_NOEXCEPT;

					// 从内部缓存移除一个浮动控件
					void UnrecordFloat(Control* ctrl)EZUI_NOEXCEPT;

					// 清空浮动控件缓存
					void ClearFloatCache()EZUI_NOEXCEPT;

					// 获取当前缓存的所有浮动控件
					const ControlCollection& GetFloatCache() const EZUI_NOEXCEPT;
				};
			private:
			...
			private:
				...
				/// 设置窗口句柄（顶层控件专用，内部使用）。
				void SetHostWindowHandle(WindowHandle hwnd)EZUI_NOEXCEPT;
			...
			protected:
				...
				virtual bool ApplyAttribute(const UIString& key, const UIString& value);
				...
				virtual bool SetStyleProperty(StyleProxy& style, const UIString& key, const UIString& value);

				/// 获取指定视觉状态下的样式（内部使用）。
				virtual StyleProxy& GetStyle(VisualState state);

				/// 获取当前控件的视觉状态。
				VisualState GetVisualState()EZUI_NOEXCEPT;
				...
				/// 文件拖拽事件统一入口（内部使用）。
				virtual void OnDragEvent(FileDragEventArgs* args);
			...
			public:
				...
				WindowHandle GetHostWindowHandle() const EZUI_NOEXCEPT;

				/// 获取承载该控件的主窗口实例（若存在）。
				Window* GetHostWindow() const EZUI_NOEXCEPT;
				...
				/// 移动控件位置（相对于父控件）。
				void SetPosition(const Point& position, float srcScale);

				/// 设置控件大小（频繁调用会影响性能，会触发 SetRect）。
				void SetSize(const Size& size);

				/// 设置控件大小（频繁调用会影响性能，会触发 SetRect）。
				void SetSize(const Size& size, float srcScale);
				...
				/// <summary>
				/// 按指定源缩放比例设置相对父控件的矩形，并返回实际应用的 rect。
				/// </summary>
				/// <param name="rect">基于源缩放比例的相对父控件矩形。</param>
				/// <param name="srcScale">rect 所使用的源缩放比例。</param>
				/// <returns>返回应用后的实际矩形（相对父控件）。</returns>
				Rect SetRect(const Rect& rect, float srcScale);

				// 获取宽度方向的尺寸策略 优先级(Fixed > Auto > Percent > Stretch)
				SizePolicy GetWidthPolicy() const EZUI_NOEXCEPT;

				// 获取高度方向的尺寸策略 优先级(Fixed > Auto > Percent > Stretch)
				SizePolicy GetHeightPolicy() const EZUI_NOEXCEPT;

				/// 获取固定宽度（未考虑比率/自动）。
				int GetFixedWidth()const EZUI_NOEXCEPT;

				/// 获取固定高度（未考虑比率/自动）。
				int GetFixedHeight()const EZUI_NOEXCEPT;

				/// 获取最小宽度约束。
				int GetMinWidth()const EZUI_NOEXCEPT;

				/// 获取最小高度约束。
				int GetMinHeight()const EZUI_NOEXCEPT;

				/// 获取最大宽度约束。
				int GetMaxWidth()const EZUI_NOEXCEPT;

				/// 获取最大高度约束。
				int GetMaxHeight()const EZUI_NOEXCEPT;

				/// 设置最小宽度。
				void SetMinWidth(int minWidth)EZUI_NOEXCEPT;

				/// 设置最小高度。
				void SetMinHeight(int minHeight)EZUI_NOEXCEPT;

				/// 设置最大宽度。
				void SetMaxWidth(int maxWidth)EZUI_NOEXCEPT;

				/// 设置最大高度。
				void SetMaxHeight(int maxHeight)EZUI_NOEXCEPT;

				/// 设置最小尺寸。
				void SetMinSize(const Size& minSize)EZUI_NOEXCEPT;

				/// 设置最大尺寸。
				void SetMaxSize(const Size& maxSize)EZUI_NOEXCEPT;

				/// 获取光标位置矩形（用于输入框等）。
				virtual Rect GetCaretRect()const;

				/// 是否自动宽度（根据内容）。
				virtual bool IsAutoWidth()const EZUI_NOEXCEPT;

				/// 是否自动高度（根据内容）。
				virtual bool IsAutoHeight()const EZUI_NOEXCEPT;
				...
				/// 分别设置自动宽高。
				virtual void SetAutoSize(bool autoWidth, bool autoHeight);

				/// 获取控件内容大小（内容区域，不含内外边距）。
				virtual Size GetContentSize()const EZUI_NOEXCEPT;

				/// 获取控件当前尺寸。
				Size GetSize()const EZUI_NOEXCEPT;

				/// 获取控件相对父控件的位置。
				Point GetPosition()const EZUI_NOEXCEPT;

				/// <summary>
				/// 获取相对于父控件的矩形。
				/// </summary>
				/// <returns>返回相对矩形位置。</returns>
				virtual Rect GetRect()const EZUI_NOEXCEPT;
				...
				/// 是否存在挂起的布局（已标记但未执行）。
				bool IsLayoutPending()const EZUI_NOEXCEPT;

				/// 标记为脏布局，等待合适时机刷新布局。
				void InvalidateLayout() EZUI_NOEXCEPT;

				/// 结束布局状态（布局更改之后必须调用此函数来结束布局状态）。
				void EndLayout()EZUI_NOEXCEPT;
				...
				/// 获取父控件指针（若存在）。
				Control* GetParent() const EZUI_NOEXCEPT;

				/// 判断当前控件是否为指定控件的子孙控件。
				bool IsChildOf(Control* parent) const EZUI_NOEXCEPT;

				/// 获取控件类名列表（用于样式选择器）。
				const std::vector<UIString>& GetClassNames()const EZUI_NOEXCEPT;

				/// 判断是否包含指定类名（用于样式匹配）。
				bool HasClass(const UIString& className)const EZUI_NOEXCEPT;

				/// 获取所有子控件集合。
				const ControlCollection& GetChildren()const EZUI_NOEXCEPT;
				...
				T* FindControl(const UIString& ctrlName) {
					...
					if (ctrl && !out) {
						...
						EZUI_ERROR(L"Control Type Error!");
					}
#endif // DEBUG
					return out;
				}
...
#ifdef EZUI_COMPILER_VS2010
				Control* FindChild(const UIString& ctrlName) {
					return this->FindChild<Control>(ctrlName);
				}
#endif // #ifdef EZUI_COMPILER_VS2010
				...
				/// 启用控件（取消禁用）。
				void SetEnabled(bool flag)EZUI_NOEXCEPT;

				/// 控件是否已启用。
				bool IsEnabled()const EZUI_NOEXCEPT;

				/// 在指定位置插入子控件；takeOwnership 为 true 时同时取得对象所有权。
				/// 弹簧控件始终跟随当前控件销毁。
				virtual Control* InsertChildAt(int pos, Control* childCtrl, bool takeOwnership = false);

				/// 添加子控件到末尾；takeOwnership 为 true 时同时取得对象所有权。
				/// 弹簧控件始终跟随当前控件销毁。
				virtual Control* AddChild(Control* childCtrl, bool takeOwnership = false);
				...
				/// 从控件树移除子控件；deleteChild 为 true 时同时立即删除该控件。
				/// deleteChild 为 false 时不改变现有对象所有权。
				virtual bool RemoveChild(Control* childCtrl, bool deleteChild = false);

				/// 移除并销毁全部弹簧控件。
				void DestroySpacers();

				/// 设置控件树父级；不改变 Object 所有权关系。
				virtual void SetParent(Control* parent);

				/// 根据下标移除子控件；deleteChild 为 true 时同时立即删除该控件。
				void RemoveChildAt(int pos, bool deleteChild = false);

				/// 移除全部子控件；deleteChildren 为 true 时同时立即删除这些控件。
				virtual void RemoveAll(bool deleteChildren = false);
				...
				/// 控件是否浮动。
				virtual bool IsFloating()const EZUI_NOEXCEPT;

				/// 设置控件可见性。
				virtual void SetVisible(bool flag);

				/// 获取控件当前可见性状态（考虑父控件影响）。
				virtual bool IsVisible()const EZUI_NOEXCEPT;

				/// 设置控件是否参与命中测试（false 则鼠标穿透）。
				void SetHitTestVisible(bool bEnable);

				/// 控件是否参与命中测试。
				bool IsHitTestVisible()const EZUI_NOEXCEPT;

				/// 设置控件自身是否对鼠标透明（自身不参与命中测试，但子控件可参与）。
				void SetMouseTransparent(bool bFlag)EZUI_NOEXCEPT;

				/// 控件自身是否为鼠标穿透状态。
				bool IsMouseTransparent()const EZUI_NOEXCEPT;

				/// 设置控件是否接收文件拖拽事件。
				void SetDropEnabled(bool value)EZUI_NOEXCEPT;

				/// 控件是否接收文件拖拽事件。
				bool IsDropEnabled()const EZUI_NOEXCEPT;
				...
				/// 设置控件指定状态下的样式属性。
				/// </summary>
				/// <param name="styleProperties">样式字符串，多个属性用分号隔开，如 "font-size:13px; color:#ffffff;"</param>
				/// <param name="state">目标状态，如 VisualState::Hover / Normal / Pressed</param>
				virtual void SetStyle(const UIString& styleProperties, VisualState state = VisualState::Normal);
				...
				/// 获取外边距信息（相对父控件）。
				Distance GetMargin()const EZUI_NOEXCEPT;
				...
				/// 获取内边距信息。
				Distance GetPadding()const EZUI_NOEXCEPT;

				/// 控件是否处于按下状态。(如果是 返回按钮 反之返回MouseButton::None)
				MouseButton PressedButton()const EZUI_NOEXCEPT;

				/// 将当前控件设置焦点或取取消焦点
				void SetFocused(bool bLogicalFocus);

				//是否具有逻辑焦点(获取内部状态)
				bool IsFocused()const EZUI_NOEXCEPT;
