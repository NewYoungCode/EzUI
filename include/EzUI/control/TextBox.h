...
namespace ezui {
	...
	{
	private:
		TextAlign m_textAlign;//文字对其方式(针对单行输入框有效)
		bool m_bCaretShow;//用于光标闪烁
		bool m_readOnly;//是否只读
		bool m_multiLine;//是否多行输入
		bool m_down;//是否具有焦点中
		bool m_focus;//是否具有焦点中
		bool m_textInteractionEnabled;//是否启用文本选择与光标交互
		...
		int m_A_TextPos;//点击了第几个字符
		int m_B_TextPos;//点击了第几个字符
		int m_textPos;//当前文字的下标 0~text.size()
		int m_scrollX;//用于左右滚动
		int m_scrollY;//用于y轴滚动
		int m_lastX;//上一个x位置
		int m_lastY;//上一个y位置
		int m_lastWidth;//上次宽度
		int m_lastHeight;//上次高度
		int m_maxLen;//最大文字数量
		Timer* m_timer;//用于光标闪烁
		VScrollBar* m_vScrollbar;//垂直滚动条
		Font* m_font;//字体
		TextLayout* m_textLayout;//字体布局
		std::wstring& m_text;//文字
		std::wstring& m_placeholder;//placeholder懂得都懂 (在没有文字的情况下显示的文字)
		std::wstring& m_passwordChar;//密码占位符
		PropertyValue<Color> m_placeholderColor;//占位符颜色
	...
	private:
		...
		bool CanUserInteractWithText() const;
		void Init();//初始化
	...
	protected:
		...
		virtual void SetAutoHeight(bool flag)EZUI_OVERRIDE;
		virtual void OnForePaint(PaintEventArgs* e) EZUI_OVERRIDE;
		virtual void OnKeyChar(KeyEventArgs* arg) EZUI_OVERRIDE;
		virtual void OnKeyDown(KeyEventArgs* arg)EZUI_OVERRIDE;
		...
		virtual void OnFocus(FocusEventArgs* arg) EZUI_OVERRIDE;
		virtual void OnKillFocus(FocusEventArgs* arg) EZUI_OVERRIDE;
		virtual void OnLayout()EZUI_OVERRIDE;
		virtual void OnScroll(int offsetX, int offsetY)EZUI_OVERRIDE;
	public:
		std::function<void(TextBox* sender, const UIString& text)> TextChanged;
	public:
		...
		//获取焦点所在光标位置
		virtual Rect GetCaretRect()const EZUI_OVERRIDE;

		//分析字符串
		void RefreshTextLayout();

		//在当前光标中插入文字(可触发文字更改事件)
		void InsertText(const UIString& str);
		...
		// 启用/禁用文本选择与光标交互（如选中文本、显示/移动光标）
		void SetTextInteractionEnabled(bool enabled);

		// 是否启用文本选择与光标交互
		bool IsTextInteractionEnabled() const;
