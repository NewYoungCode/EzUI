...
namespace ezui {
	...
	struct DumpConfig
	{
		bool dumpUiTree = false;
		bool dumpScreenshot = false;
		std::wstring uiFile = L"dbg_layout.json";
		std::wstring shotFile = L"dbg_capture.png";
	};
#endif // DEBUG
	 ...
	 * - 本类大多数方法应在 UI（主）线程调用。
	 */
	class EZUI_API Window : public Object
	{
	...
	private:
		struct WindowContext {
			...
			/// 当前具有鼠标焦点的控件
			CtrlWeakPtr<Control> m_hoverControl = NULL;
			/// 当前具有键盘输入焦点的控件
			CtrlWeakPtr<Control> m_focusedControl = NULL;
			// 窗口桥接对象指针（用于 Control 与 Window 之间的通信）
			// 通过该对象调用窗口相关能力（如重绘、刷新、设置焦点等）
			WindowBridge* m_windowBridge = NULL;
			/// 当前具有焦点的滚动条（若有）
			CtrlWeakPtr<ScrollBar> m_scrollBar = NULL;
			/// 原生窗口句柄（WindowId）
			...
			int64_t m_lastDownTime = 0;
			/// 窗口最小尺寸约束
			Size m_minSize;
			/// 窗口最大尺寸约束
			Size m_maxSize;
			/// 所属（Owner）窗口句柄，若无则为 NULL
			WindowHandle m_ownerWnd = NULL;
			/// 窗口根 Frame（窗口内容根容器 负责加载布局文件 内部使用）
			Frame* m_rootFrame = NULL;
			//缩放率
			float Scale = 1.0f;
		};
	private:
	...
	protected:
		...
		/// 窗口关闭前回调，可通过 cancel 控制是否允许关闭（true 表示取消关闭）
	...
	public:
		...
		void SetTopAt(WindowHandle targetHwnd);
		...
		/// 获取所属窗口句柄（Owner）
		WindowHandle GetOwnerWindow() noexcept;
		...
		/// 获取基于屏幕的矩形位置
		Rect GetRectInScreen()const noexcept;
		...
		/// 设置窗口主布局控件（Frame/Panel）
		void SetLayout(ezui::Control* layout, bool autoDelete = false);

		/// 从 XML 文件加载并应用布局到窗口（路径为工程可访问路径）
		void LoadXml(const UIString& filePath);

		/// 获取窗口主布局控件（若未设置返回 NULL）
		Control* GetLayout()const noexcept;

		/// 设置窗口标题文字
		void SetText(const UIString& text);

		/// 获取窗口标题文字
		UIString GetText()const noexcept;
		...
		Control* GetFocusControl()const noexcept;

		// 是否启用 Alpha 混合（Windows 下即分层窗口）
		bool IsAlphaBlendingEnabled()const noexcept;
