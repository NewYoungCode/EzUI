...
namespace ezui {
	...
	namespace detail {
		inline void EZUI_OutputAssert(const wchar_t* expr, const wchar_t* file, unsigned int line, bool dbgBreak = true)
		{
			...
			int len = std::swprintf(nullptr, 0, msg.c_str(), safeFile, line, safeExpr);
...
//断言宏 如果表达式为假 则输出断言信息并且触发断点
#define EZUI_ASSERT(expression) ((void)(                                                       \
            (!!(expression)) ||                                                               \
            (ezui::detail::EZUI_OutputAssert(_CRT_WIDE(#expression), _CRT_WIDE(__FILE__), (unsigned)(__LINE__), true), 0)) \
        )

//输出指定信息但不触发断点(只打印日志)
#define EZUI_INFO(msg_wchar_t) \
    ((void)( \
