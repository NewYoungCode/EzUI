...
namespace ezui {
	...
	struct DisplayInfo {
		...
		ezui::Rect WorkRect;
		DisplayInfo()
			: Scale(1.0f), FPS(60), Primary(false), Monitor(NULL) {
		}
		//显示是否可用
		bool IsValid() const {
			return Monitor != NULL;
	...
	extern EZUI_API int64_t GetTickCountMs();
	// 睡眠指定毫秒数，并返回实际睡眠的毫秒数
	extern EZUI_API int64_t SleepMs(int64_t ms);
	//复制内容到剪切板
	...
	namespace detail {
		...
		public:
			ContentMeasure()EZUI_NOEXCEPT;
			void Update(int left, int top, int right, int bottom)EZUI_NOEXCEPT;
			void Update(const Rect& rect, const Distance& margin)EZUI_NOEXCEPT;
			void UpdateHorizontal(const Rect& rect, const Distance& margin) EZUI_NOEXCEPT;
			void UpdateVertical(const Rect& rect, const Distance& margin) EZUI_NOEXCEPT;
			int ContentWidth() const EZUI_NOEXCEPT;
			int ContentHeight() const EZUI_NOEXCEPT;
		};
	};
